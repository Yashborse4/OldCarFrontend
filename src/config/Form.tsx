// Theme is provided via ThemeContext
...
const FieldComponent: React.FC<FieldProps> = ({
  ...
  const renderInput = () => {
    const mappedVariant = variant === 'outlined'
      ? 'outline'
      : variant === 'filled'
        ? 'filled'
        : variant === 'underlined'
    ...
    const mappedSize = size === 'small' ? 'sm' : size === 'large' ? 'lg' : 'md';
    ...
    const commonProps = {
      ...
      placeholder,
      error: displayError,
      hint: displayError ? undefined : helperText,
  ...
  const getButtonStyles = () => {
    ...
    switch (variant) {
      case 'filled':
        return {
          ...baseStyle,
          backgroundColor: isDisabled ? theme.colors.textDisabled + '20' : theme.colors.primary,
