    ...
    switch (position) {
      case 'top':
        return {
          ...
          paddingTop: insets.top + getResponsiveSpacing('lg'),
        };
      case 'bottom':
        return {
          justifyContent: 'flex-end',
          paddingBottom: insets.bottom + getResponsiveSpacing('lg'),
        };
      case 'fullScreen':
        return {
          margin: 0,
...
const DialogComponent: React.FC<DialogProps> = ({
      ...
      <View style={[styles.dialogActions, actionsStyle]}>
        {actions.map((action, index) => (
          <TouchableOpacity
            ...
            style={[
              styles.dialogAction,
              action.style === '' && { backgroundColor: '#FFD700' },
              action.style === 'destructive' && { backgroundColor: '#F56565' },
          ...
          >
            <Text
              style={[
                ...
                {
                  color: action.style === '' || action.style === 'destructive'
                    ? '#FFFFFF' : '#FFD700',
                },
              ]}
            >
    ...
    <View style={styles.dialogContainer}>
      ...
      <View
        style={[
          ...
          maxHeight !== undefined ? ({ maxHeight: maxHeight } as ViewStyle) : undefined,
        ]}
      >
        {scrollable ? (
          <ScrollView showsVerticalScrollIndicator={false}>
            {message && (
              <Text style={[styles.dialogMessage, { color: '#4A5568' }, messageStyle]}>
                {message}
              </Text>
            )}
            {content}
  ...
  return (
    <BaseModal
      ...
      animationType="scale"
      position="center"
      contentStyle={StyleSheet.flatten([styles.dialogWrapper, { backgroundColor: '#FFFFFF' }])}
    >
      {dialogContent}
    </BaseModal>
  );
      ...
      {(title || subtitle || headerLeft || headerRight || showCloseButton) && (
        <View style={[styles.bottomSheetHeader, headerStyle]}>
          ...
          <View style={styles.headerCenter}>
            ...
            {subtitle && (
              <Text style={[styles.bottomSheetSubtitle, { color: '#4A5568' }, subtitleStyle]}>
                {subtitle}
              </Text>
            )}
          </View>
...
const LoadingDialogComponent: React.FC<LoadingDialogProps> = ({
  ...
  return (
    <BaseModal
      ...
      contentStyle={StyleSheet.flatten([styles.loadingDialogWrapper, { backgroundColor: '#FFFFFF' }])}
    >
      <View style={styles.loadingDialogContent}>
        ...
        {showProgress && typeof progress === 'number' && (
          <View style={styles.progressContainer}>
            ...
            <Text style={[styles.progressText, { color: '#4A5568' }]}>
              {Math.round(progress)}%
            </Text>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  bottomSheetWrapper: {
    ...
    borderTopRightRadius: getResponsiveBorderRadius('lg'),
    maxHeight: '90%',
    minHeight: '20%',
  },
  bottomSheetHandle: {
